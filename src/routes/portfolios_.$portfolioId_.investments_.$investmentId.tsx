...
function InvestmentDetailPage() {
  const { portfolioId, investmentId } = Route.useParams()
  const {
    data: investment,
    isLoading,
    error,
  ...
  if (error || !investment) {
    return <InvestmentDetailError portfolioId={portfolioId} />
  }
...
function InvestmentDetailError({ portfolioId }: { portfolioId: string }) {
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center gap-4">
        <Link to="/portfolios/$portfolioId" params={{ portfolioId }}>
